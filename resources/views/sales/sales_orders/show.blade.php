        ...
        <div class="col-md-11">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-dark d-flex justify-content-between align-items-center py-3">
                    ...
                    @if ($salesOrder->status == 'Draft')
                        <form action="{{ route('sales-orders.submit', $salesOrder->id) }}" method="POST" class="d-inline">
                            @csrf
            ...
            <div class="card-body text-sm">
                            ...
                            <table class="table table-bordered table-striped table-sm text-sm">
                                <thead class="bg-dark color-white text-center text-xs uppercase">
                                    <tr>
                                        ...
                                        <th width="100" class="hide-mobile">Unit</th>
                                        <th width="120">Quantity</th>
                                        <th width="150">Unit Price</th>
                                        <th width="120" class="hide-mobile">Tax</th>
                                    ...
                                    @foreach ($salesOrder->lines as $index => $line)
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                ...
                                                <small class="text-muted uppercase hide-mobile">SKU:
                                            ...
                                            <td class="text-right align-middle px-3 text-muted hide-mobile">Rp
                ...
                @if ($approval)
                    <div class="row px-2 mb-4">
                        <div class="col-12">
                            <div
                                class="alert alert-{{ $approval->status == 'Approved' ? 'success' : ($approval->status == 'Rejected' ? 'danger' : 'warning') }} border shadow-sm">
                                <h6 class="font-weight-bold"><i class="fas fa-shield-alt mr-1"></i> Approval Status:
                                    {{ $approval->status }}</h6>
                                <p class="mb-1 small">Requested by: {{ $approval->requestedBy->name }} on
                                    {{ $approval->created_at->format('d M Y H:i') }}</p>
                                @if ($approval->approved_by)
                                    <p class="mb-1 small">Processed by: {{ $approval->approvedBy->name }}</p>
                                @endif
                                @if ($approval->notes)
                                    <p class="mb-0 small"><strong>Notes:</strong> {{ $approval->notes }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="card-footer bg-white border-top text-right py-3">
                ...
                @if ($salesOrder->status == 'Draft')
                    <form action="{{ route('sales-orders.submit', $salesOrder->id) }}" method="POST" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-primary btn-sm ml-2 px-4 font-weight-bold shadow-sm">
                            <i class="fas fa-paper-plane mr-1"></i> Submit for Approval
                        </button>
                    </form>
                @endif
